...
The unchanged `scripts/plan-exec.py` owns staging, structural checks,
...
Relative project executables must be executable boundary-tree files;
resolve ignored local tools and virtualenv entry points absolutely.
Python import probes must resolve beneath `AI_SKILLZ_BOUNDARY_ROOT`.
