...
- Hard-max: 67 display-cols (fill width, same as `tractor`).
...
- Max line length: 67 display-cols (fill width, match `tractor`).
...
The default footer credits the active AI coding harness and
model/provider for helping generate the commit message content:

```
(this commit msg was generated in some part by `<harness>` using `<model>` (`<provider>`))
```

If the patch itself was written (in whole or part) by the AI
coding harness, use instead:

```
(this patch was generated in some part by `<harness>` using `<model>` (`<provider>`))
```

Observed in 75% of the 50 most recent commits under the legacy
`claude-code` footer format.
...
- [ ] Body lines <=67 display-cols (fill width)
...
(this style guide was generated by `claude-code` using an
unknown model/provider while analyzing commit history)
